...
**Status:** Public-export draft for Product Owner review. Not published by the export step.  
...
Reservedge is a privacy-first buyer agent: an arbitrary free-form objective becomes clarification, a labelled multi-task plan, human confirmation, then **simulated** parking execution behind distinct human grants A1–A4.
...
- Parking after plan confirm uses the existing governed façade (A1–A4, isolated simulated suppliers, locked JFK ranking vector).
...
- External booking-API integrations (later workstream)
...
```bash
ITAA_AWS_MODEL_MODE=fake \
ITAA_AWS_ADAPTER_URL=http://127.0.0.1:8080 \
uv run uvicorn itaa_api.app:app --app-dir apps/api/src --host 127.0.0.1 --port 8011
```
...
- Public GitHub push / Devpost submit
- Final live-demo recording
- AgentCore deploy
- External booking API implementation
